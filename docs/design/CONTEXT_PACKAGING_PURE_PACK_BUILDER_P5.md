...
P1c normalization is deliberately narrow. Only canonical-state `standing_evidence[*].raw_sha256` values are lowercase-normalized before identical evidence identities are deduplicated and JCS-sorted. Other source-reference, source-binding, and toolchain identity spellings remain identity-bearing and are not silently rewritten.
