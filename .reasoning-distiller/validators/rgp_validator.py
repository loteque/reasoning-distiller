#!/usr/bin/env python3
"""Deterministic structural validator for rgp/1 graphs and submission envelopes."""
...
def _validate_provenance(value: Any, path: str, errors: list[str]) -> bool:
    ...
    for role, refs in value.items():
        ...
        if isinstance(refs, list):
            ...
            if overlap:
                _error(errors, f"{path}.{role}", f"source references repeated across provenance roles: {sorted(overlap)}")
                ok = False
            seen.update(refs)
    return ok


def _extract_graph(document: Any, errors: list[str]) -> Any:
    if not isinstance(document, dict):
        return document
    if "candidate_graph" not in document:
        return document
    if document.get("rgp_version") != "rgp/1":
        _error(errors, "$.rgp_version", "submission envelope must declare rgp/1")
    if document.get("status") != "candidate":
        _error(errors, "$.status", "submission envelope status must be candidate")
    graph = document.get("candidate_graph")
    if not isinstance(graph, dict):
        _error(errors, "$.candidate_graph", "must be an object")
    return graph


def validate(document: Any) -> list[str]:
    errors: list[str] = []
    graph = _extract_graph(document, errors)
    if not isinstance(graph, dict):
        errors.append("$: RGP graph must be an object")
        return errors

    unknown = set(graph) - GRAPH_KEYS
    if unknown:
        _error(errors, "$.candidate_graph" if graph is not document else "$", f"unknown fields: {sorted(unknown)}")
    ...
    for index, relation in enumerate(relations or []):
        ...
        if isinstance(source, str) and isinstance(target, str) and isinstance(relation_type, str):
            ...
            if key in relation_keys:
                _error(errors, path, "duplicate relation")
            relation_keys.add(key)
            if relation_type == "contradicts":
                pair = tuple(sorted((source, target)))
                if pair in contradiction_pairs:
                    _error(errors, path, "duplicate symmetric contradiction")
...
def main() -> int:
    ...
    if args.version:
        ...
        return 0
    if not args.paths:
        parser.error("at least one path is required unless --version is used")
