...
def grant_payload(
    *,
    ...
    if not isinstance(operations, list) or not operations or any(not isinstance(v, str) or not v for v in operations):
        raise ContractError("INVALID_GRANT_SCOPE", "operations must be a non-empty explicit list")
    if len(set(operations)) != len(operations):
        raise ContractError("INVALID_GRANT_SCOPE", "operations must be unique")
    for operation_class in operations:
        meta = _gov.delegation_metadata(operation_class)
        if not meta.get("delegable"):
    ...
    value = {
        ...
        "scope": {
            ...
            "targets": sorted(targets, key=lambda v: canonical_json_bytes(v)),
            "constraints": sorted(constraints, key=lambda v: canonical_json_bytes(v)),
        },
...
def validate_event(event: dict[str, Any]) -> None:
    ...
    if not isinstance(event, dict) or set(event) != required or event.get("contract") != GRANT_EVENT_CONTRACT:
        raise ContractError("INVALID_GRANT_EVENT", "grant event fields do not match contract")
    if event["event_type"] not in CORE_EVENTS:
        raise ContractError("INVALID_GRANT_EVENT", "unsupported grant event")
...
def project_grant(store: Path, grant_ref: str, *, workflow_lifecycle: str = "OPEN") -> dict[str, Any]:
    ...
    revoked = next((e for e in events if e["event_type"] == "core/revoked"), None)
    exhausted = next((e for e in events if e["event_type"] == "core/exhausted"), None)
    ...
    if workflow_lifecycle in TERMINAL_WORKFLOW_LIFECYCLES:
        ...
        state = "REVOKED"
    elif exhausted is not None or (limit is not None and len(issued) >= limit):
        state = "EXHAUSTED"
    else:
        state = "ACTIVE"
    remaining = None if limit is None else max(0, limit - len(issued))
    return {
...
def validate_scope(
    ...
    *,
    ...
    if operation_class not in grant["payload"]["scope"]["operations"]:
        return {"classification": "OUTSIDE_GRANT", "grant": grant_ref}
    if not workflow_contains_proposal:
        return {"classification": "OUTSIDE_GRANT", "grant": grant_ref}
    if set(authority_fields) != set(metadata.get("target_fields", [])) | set(metadata.get("constraints", {}).keys()):
        return {"classification": "OUTSIDE_GRANT", "grant": grant_ref}
...
def issue_approval(
    ...
    *,
    ...
    # Reserve event identity first because approval/2 binds the exact issuance event.
    events = read_events(store, grant_ref)
    sequence = len(events) + 1
    proposal_digest = digest(proposal)
    event_payload_stub = {"proposal_digest": proposal_digest, "approval_digest": None}
    body_stub = {
        "contract": GRANT_EVENT_CONTRACT,
        "grant": grant_ref,
        "sequence": sequence,
        "event_type": "core/approval-issued",
        "expected_normative_head": expected_normative_head,
        "payload": event_payload_stub,
        "authentication": None,
    }
    # Break the approval/event hash cycle by defining event identity over the proposal and
    # predecessor; approval_digest is integrity metadata and is excluded from reference derivation.
    event_ref = "authority-grant-event:" + digest({k: v for k, v in body_stub.items() if k != "payload"} | {"payload": {"proposal_digest": proposal_digest}}).split(":", 1)[1]
    ...
    event = {
        ...
        "event_type": "core/approval-issued",
        "expected_normative_head": expected_normative_head,
        "payload": {"proposal_digest": proposal_digest, "approval_digest": digest(approval)},
        "authentication": None,
    }
    _validate_issued_event_identity(event)
    _write_exclusive(_grant_dir(store, grant_ref) / "events" / f"{sequence:08d}.json", canonical_json_bytes(event))
...
def revoke_grant(store: Path, grant_ref: str, operator_id: str, authentication: dict[str, Any], *, protected_root: bool = False, expected_normative_head: str | None) -> str:
    ...
    required_confirmation = "ROOT_REVOKE_AUTHORITY_GRANT" if protected_root and operator_id != grant["payload"]["grantor"] else "REVOKE_AUTHORITY_GRANT"
    if not isinstance(authentication, dict) or authentication.get("operator_id") != operator_id or authentication.get("subject") != grant_ref or authentication.get("confirmation") != required_confirmation or not authentication.get("method"):
...
def _append_event(store: Path, grant_ref: str, event_type: str, payload: dict[str, Any], *, expected_normative_head: str | None, authentication: dict[str, Any] | None) -> str:
    ...
    if current != expected_normative_head:
        raise ContractError("GRANT_NORMATIVE_HEAD_CONFLICT", "grant normative head changed")
    sequence = len(events) + 1
    body = {"contract": GRANT_EVENT_CONTRACT, "grant": grant_ref, "sequence": sequence, "event_type": event_type, "expected_normative_head": current, "payload": payload, "authentication": authentication}
...
def _validate_issued_event_identity(event: dict[str, Any]) -> None:
    required = {"contract", "reference", "grant", "sequence", "event_type", "expected_normative_head", "payload", "authentication"}
    if set(event) != required or event["event_type"] != "core/approval-issued":
        raise ContractError("INVALID_GRANT_EVENT", "invalid approval-issued event")
    identity_body = {k: event[k] for k in required if k not in {"reference", "payload"}}
    identity_body["payload"] = {"proposal_digest": event["payload"]["proposal_digest"]}
    expected = "authority-grant-event:" + digest(identity_body).split(":", 1)[1]
    if event["reference"] != expected:
        raise ContractError("GRANT_EVENT_IDENTITY_MISMATCH", "approval-issued reference mismatch")
    canonical_json_bytes(event)
...
def _targets_match(selectors: list[dict[str, Any]], metadata: dict[str, Any], fields: dict[str, Any]) -> bool:
    ...
    for field in metadata.get("target_fields", []):
        ...
        if selector is None:
            return False
        allowed = metadata.get("selectors", {}).get(field, [])
        if selector["match"] not in allowed:
            return False
        ...
        if selector["match"] == "one-of" and actual not in selector.get("values", []):
            return False
        if selector["match"] == "within":
            # v1 requires the caller to provide canonical ancestry explicitly.
            ancestry = fields.get(field + "__parents", [])
            if selector.get("value") not in ancestry:
                return False
    return True
...
def _constraints_match(constraints: list[dict[str, Any]], metadata: dict[str, Any], fields: dict[str, Any]) -> bool:
    for constraint in constraints:
        ...
        if predicate not in metadata.get("constraints", {}).get(field, []):
            return False
        actual = fields.get(field)
        expected = constraint.get("value")
        if predicate == "eq" and actual != expected:
            return False
        if predicate == "one-of" and actual not in constraint.get("values", []):
            return False
        if predicate == "max-count" and (not hasattr(actual, "__len__") or len(actual) > expected):
            return False
        if predicate == "subset-of" and not set(actual).issubset(set(constraint.get("values", []))):
            return False
    return True
...
def _validate_selector(value: dict[str, Any]) -> None:
    ...
    match = value["match"]
    if match in {"exact", "within"} and set(value) != {"field", "match", "value"}:
        raise ContractError("INVALID_GRANT_SCOPE", "selector fields do not match")
    if match == "one-of" and (set(value) != {"field", "match", "values"} or not isinstance(value.get("values"), list) or not value["values"]):
...
def _validate_constraint(value: dict[str, Any]) -> None:
    ...
    if pred in {"eq", "max-count"} and set(value) != {"field", "predicate", "value"}:
        raise ContractError("INVALID_GRANT_SCOPE", "constraint fields do not match")
    if pred in {"one-of", "subset-of"} and (set(value) != {"field", "predicate", "values"} or not isinstance(value.get("values"), list)):
        raise ContractError("INVALID_GRANT_SCOPE", "constraint values must be finite list")
