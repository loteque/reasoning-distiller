...
def make_proposal(domain: str, operation: str, current_state: Any, change: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "domain": domain,
        "operation": operation,
        "basis_digest": digest(current_state),
        "change": change,
    }
...
def make_approval(proposal: dict[str, Any], operator_id: str, authentication: dict[str, Any]) -> dict[str, Any]:
    ...
    if not operator_id.startswith("operator:"):
...
def validate_proposal(proposal: dict[str, Any]) -> None:
    ...
    if proposal["contract"] != PROPOSAL_CONTRACT:
        raise ContractError("INVALID_PROPOSAL", "unsupported proposal contract")
    if not proposal["domain"] or not proposal["operation"]:
        raise ContractError("INVALID_PROPOSAL", "domain and operation are required")
...
def validate_approval(approval: dict[str, Any], proposal: dict[str, Any]) -> None:
    required = {"contract", "proposal_digest", "operator_id", "authentication"}
    if not isinstance(approval, dict) or set(approval) != required:
        raise ContractError("INVALID_APPROVAL", "approval fields do not match contract")
    ...
    if not approval["operator_id"].startswith("operator:"):
        raise ContractError("INVALID_APPROVAL", "operator_id must use operator: namespace")
    auth = approval["authentication"]
    if not isinstance(auth, dict) or not auth.get("method"):
        raise ContractError("INVALID_APPROVAL", "authentication.method is required")
...
def load_json(path: Path) -> Any:
    ...
    try:
        ...
        raise ContractError("INVALID_JSON", f"invalid JSON at {path}") from exc
    if raw != canonical_json_bytes(value):
        raise ContractError("NON_CANONICAL_ARTIFACT", f"artifact is not canonical JSON: {path}")
    return value
...
def replay(events_dir: Path, initial_state: Any | None = None) -> tuple[Any, list[dict[str, Any]]]:
    ...
    for expected, path in enumerate(paths, start=1):
        if path.name != f"{expected:08d}.json":
            raise ContractError("EVENT_SEQUENCE_CONFLICT", "event sequence is not contiguous")
        event = load_json(path)
        required = {
            "contract", "sequence", "domain", "operation", "proposal_digest",
            "approval_digest", "basis_digest", "result_digest", "result_state"
        }
        if not isinstance(event, dict) or set(event) != required:
            raise ContractError("INVALID_EVENT", f"invalid event fields: {path.name}")
        if event["contract"] != EVENT_CONTRACT or event["sequence"] != expected:
            raise ContractError("INVALID_EVENT", f"invalid event contract/sequence: {path.name}")
        if event["basis_digest"] != digest(state):
            raise ContractError("EVENT_CHAIN_CONFLICT", f"basis mismatch at {path.name}")
        if event["result_digest"] != digest(event["result_state"]):
            raise ContractError("EVENT_CHAIN_CONFLICT", f"result mismatch at {path.name}")
        key = (event["proposal_digest"], event["approval_digest"])
        if key in consumed:
            raise ContractError("APPROVAL_REUSE", f"approval/proposal pair reused at {path.name}")
        consumed.add(key)
...
def projection_status(events_dir: Path, projection_path: Path, initial_state: Any | None = None) -> dict[str, Any]:
    try:
        ...
        return {
            "contract": PROJECTION_STATUS_CONTRACT,
            "status": "CONFLICT",
    ...
    if not projection_path.is_file() or projection_path.is_symlink():
        return {
            "contract": PROJECTION_STATUS_CONTRACT,
            "status": "CONFLICT",
            "reason_code": "PROJECTION_PATH_CONFLICT",
        }
    try:
    ...
    if digest(projection) != replay_digest:
        return {
            "contract": PROJECTION_STATUS_CONTRACT,
            "status": "CONFLICT",
...
def rebuild_projection(events_dir: Path, projection_path: Path, initial_state: Any | None = None) -> dict[str, Any]:
    ...
    if status["status"] == "CONFLICT":
        return operation_result("FAIL", "PROJECTION_CONFLICT", status.get("detail"), projection_status=status)
    if status["status"] == "VALID":
        return operation_result("PASS", "NO_CHANGE", projection_digest=status["replay_digest"])
    state, _ = replay(events_dir, initial_state)
    _write_replace(projection_path, canonical_json_bytes(state))
...
def apply_transition(
    *,
    ...
    try:
        ...
        current, events = replay(events_dir, initial_state)
        proposal_digest = digest(proposal)
        approval_digest = digest(approval)

        consumed_event = next(
            (e for e in events if e["proposal_digest"] == proposal_digest and e["approval_digest"] == approval_digest),
            None,
        )
        if consumed_event is not None:
            if digest(current) == consumed_event["result_digest"]:
                if status["status"] == "REBUILDABLE":
                    _write_replace(projection_path, canonical_json_bytes(current))
        ...
        if proposal["basis_digest"] != digest(current):
            return operation_result("FAIL", "STALE_BASIS")

        result_state = transition(current, proposal["change"])
        canonical_json_bytes(result_state)
        ...
        event = {
            ...
            "basis_digest": digest(current),
            "result_digest": digest(result_state),
            "result_state": result_state,
        }
        _write_exclusive(_event_path(events_dir, sequence), canonical_json_bytes(event))
        _write_replace(projection_path, canonical_json_bytes(result_state))
        return operation_result(
            "PASS",
            "APPLIED",
            event_sequence=sequence,
            event_digest=digest(event),
            projection_digest=digest(result_state),
        )
    except ContractError as exc:
        return operation_result("FAIL", exc.code, exc.detail)
    except FileExistsError:
        return operation_result("FAIL", "EVENT_APPEND_CONFLICT")
