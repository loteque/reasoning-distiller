...
from ril_steward_authorization import apply_authorization_change, approve_authorization_change, plan_authorization_change  # noqa: E402
from ril_storage_verification import verify_storage  # noqa: E402


class AdmissionR13Tests(unittest.TestCase):
    def identity(self) -> dict[str, str]:
        return {"id":"example-project","name":"Example Project","repository":"example/project","summary":"Admission test project."}

    def root(self) -> Path:
        root=Path(tempfile.mkdtemp()); (root/"project-knowledge/submissions").mkdir(parents=True)
    ...
    def establish(self,root:Path)->None:
        plan=plan_initial_operator(root,"operator:owner"); approval=approve_initial_operator(plan["proposal"],"operator:owner")
        self.assertEqual(apply_initial_operator(root,plan["proposal"],approval)["status"],"PASS")

    def auth(self,root:Path,scope:str)->None:
        plan=plan_authorization_change(root,"AUTHORIZE",scope,"steward:default"); approval=approve_authorization_change(plan["proposal"],"operator:owner")
    ...
    def ready(self,recommendation:str="RECOMMEND"):
        root=self.root(); self.establish(root); self.auth(root,"semantic_reconciliation")
        candidate=root/"project-knowledge/submissions/candidate.json"; candidate.write_bytes(canonical_json_bytes({"contract":"test-candidate/1","claim":"x"}))
        ...
        result=reconcile_candidate(root,candidate,reconciliation_activation,assessment); self.assertEqual(result["status"],"PASS")
    ...
    def plan(self,root:Path,base:dict|None=None,record_id:str="record:1")->dict:
        ...
        return {"contract":PLAN_CONTRACT,"expected_base_sha256":sha256_bytes(jcs(base)),"reuse_record_ids":[],"record_updates":[],"new_records":[{"id":record_id,"kind":"proposition","lifecycle":"current","data":{"statement":"x","proposition_kind":"claim","epistemic_role":"asserted"}}],"new_relations":[]}
    ...
    def test_recommendation_not_authority_and_defer_is_blocked(self):
        root,_,disposition=self.ready("DEFER"); self.auth(root,"admission"); result=admit(root,disposition,self.activation(),self.plan(root))
    ...
    def test_first_admission_requires_explicit_project_identity(self):
        root,_,disposition=self.ready(); self.auth(root,"admission")
        ...
        self.assertEqual((result["status"],result["outcome"]),("FAIL","PROJECT_IDENTITY_REQUIRED")); self.assertFalse((root/"project-knowledge/canonical").exists())

    def test_success_writes_schema_valid_project_seeded_pems_cove_and_evidence(self):
        root,_,disposition=self.ready(); self.auth(root,"admission"); result=admit(root,disposition,self.activation(),self.plan(root))
    ...
    def test_exact_retry_is_no_change(self):
        ...
        self.assertEqual(admit(root,disposition,activation,plan)["outcome"],"ADMITTED"); self.assertEqual(admit(root,disposition,activation,plan)["outcome"],"NO_CHANGE")

    def test_candidate_change_after_reconciliation_is_rejected(self):
        root,candidate,disposition=self.ready(); self.auth(root,"admission"); candidate.write_bytes(canonical_json_bytes({"contract":"test-candidate/1","claim":"changed"}))
    ...
    def test_incomplete_canonical_pair_is_rejected_before_admission(self):
        ...
        (canonical/"pems2.jcs.json").write_bytes(jcs(first_admission_base(root)))
        result=admit(root,disposition,self.activation(),self.plan(root)); self.assertEqual((result["status"],result["outcome"]),("FAIL","INCOMPLETE_CANONICAL_PAIR"))

    def test_conflicting_second_admission_is_rejected(self):
        root,_,disposition=self.ready(); self.auth(root,"admission"); activation=self.activation(); plan=self.plan(root)
        self.assertEqual(admit(root,disposition,activation,plan)["outcome"],"ADMITTED")
        changed=dict(plan); changed["new_records"]=[{"id":"record:2","kind":"proposition","lifecycle":"current","data":{"statement":"y","proposition_kind":"claim","epistemic_role":"asserted"}}]
        result=admit(root,disposition,activation,changed); self.assertEqual((result["status"],result["outcome"]),("FAIL","ADMISSION_CONFLICT"))
