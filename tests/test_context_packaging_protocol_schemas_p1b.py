...
R=Path(__file__).resolve().parents[1]; S=R/'schemas'; F=R/'tests/fixtures/context-packaging-protocol-schema-p1b.json'; MF=R/'tests/fixtures/context-packaging-protocol-schema-p1b-multi-profile.json'; P0=R/'tests/fixtures/context-packaging-pressure-cases-v1.json'; PEMS=R/'backends/pems-cove/pems-v2.schema.json'
...
class P1b(unittest.TestCase):
 @classmethod
 def setUpClass(c):
  c.sc={n:load(S/n)for n in FILES};c.fx=load(F);c.mp=load(MF);c.p0=load(P0);p=load(PEMS);reg=Registry().with_resources([(x['$id'],Resource.from_contents(x))for x in[*c.sc.values(),p]]);c.v={n:Draft202012Validator(x,registry=reg)for n,x in c.sc.items()};c.e=c.fx['examples'];c.reg=reg
