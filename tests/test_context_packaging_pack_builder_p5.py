...
def test_only_standing_evidence_digest_case_is_canonicalized():
    fx = _fixture()
    ...
    fx['projected'] = [ProjectedKnowledge(canonical_snapshot_ref=projected_ref, pems=projected.pems, causes=projected.causes)]
    fx['request_raw'] = json.dumps(fx['request'], ensure_ascii=False, indent=2, sort_keys=True).encode('utf-8')
    result = _build(fx)
    assert result.ok
    packed_binding = next(item for item in result.pack['source_registry'] if item['source_class'] == 'canonical_state')
    packed_ref = result.pack['knowledge_plane']['items'][0]['canonical_snapshot_ref']
    assert packed_binding['pems_sha256'] == upper_pems
    assert packed_ref['pems_sha256'] == upper_pems
    assert packed_binding['standing_evidence'][0]['raw_sha256'] == 'sha256:' + 'b' * 64

def test_toolchain_digest_spelling_is_identity_bearing_not_silently_normalized():
    fx = _fixture()
    validator = next(item for item in fx['components'] if item['role'] == 'pems_validator')
    upper = 'sha256:' + validator['raw_sha256'][7:].upper()
    validator['raw_sha256'] = upper
    result = _build(fx)
    assert result.ok
    packed = next(item for item in result.pack['toolchain']['components'] if item['role'] == 'pems_validator')
    assert packed['raw_sha256'] == upper
