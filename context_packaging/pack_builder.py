...
def _validate_and_canonicalize_toolchain(profile, knowledge_items, components):
    normalized = [_normalize_sha_fields(deepcopy(dict(c))) for c in components]
    ...
    for component in normalized:
        ...
        component['raw_sha256'] = _normalize_sha256(component['raw_sha256'])
    ...
    if actual['contract'] != closure['contract'] or actual['immutable_identity'] != closure['immutable_snapshot_id'] or actual['raw_sha256'] != _normalize_sha256(closure['raw_sha256']) or (by_role['jcs_serializer']['contract'] != 'jcs/1') or (by_role['pack_builder']['contract'] != PACK_BUILDER_CONTRACT):
...
def _canonical_binding(binding):
    out = _normalize_sha_fields(deepcopy(dict(binding)))
    if out.get('source_class') == 'canonical_state':
        ...
        unique = {_jcs(_normalize_sha_fields(deepcopy(item))): _normalize_sha_fields(deepcopy(item)) for item in evidence}
        out['standing_evidence'] = [unique[key] for key in sorted(unique)]
    return out

def _normalize_snapshot_ref(ref):
    out = _normalize_sha_fields(deepcopy(dict(ref)))
    if out.get('source_class') == 'canonical_state' and 'standing_evidence' in out:
        unique = {_jcs(_normalize_sha_fields(deepcopy(item))): _normalize_sha_fields(deepcopy(item)) for item in out['standing_evidence']}
        out['standing_evidence'] = [unique[key] for key in sorted(unique)]
    return out
...
def _normalize_sha_fields(value):
    if isinstance(value, list):
        return [_normalize_sha_fields(item) for item in value]
    if not isinstance(value, dict):
        return value
    out = {}
    for key, item in value.items():
        if isinstance(item, str) and key.endswith('_sha256'):
            out[key] = _normalize_sha256(item)
        else:
            out[key] = _normalize_sha_fields(item)
    return out
